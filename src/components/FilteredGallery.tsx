...
const FilteredGallery = ({
  ...
  // Type tabs: derived from provided types or registry, filtered to those present in items.
  const presentTypes = useMemo(() => {
    const registry = types ?? defaultTypes;
    const present = new Set(items.map((i) => i.type));
    return registry.filter((t) => present.has(t.key));
  }, [items, types]);

  // Items remaining after applying type filter only — used to derive tag list & counts.
  ...
  }, [typeFiltered, selectedTags, matchMode]);
  ...
  // Reset tags that no longer apply when switching types
  ...
  return (
    <div className="min-w-0">
      ...
      {visible.length > 0 ? (
        <CardScroller key={`${activeType}-${selectedTags.join(",")}-${matchMode}`} items={visible} columns={columns} rows={rows} />
      ) : (
