...
for (const file of blogFiles) {
  ...
  fs.writeFileSync(path.join(compiledBlogDir, `${slug}.html`), htmlBody, 'utf8');
}

// Sort posts by date descending
postsMetadata.sort((a, b) => new Date(b.date) - new Date(a.date));

// Write consolidated posts.json
...
for (const file of pageFiles) {
  ...
  // Strip frontmatter if present, otherwise treat entire file as body
  const frontmatterRegex = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/;
  const match = rawContent.match(frontmatterRegex);
  ...
  const htmlBody = pandocProcess.stdout.toString('utf8');
  fs.writeFileSync(path.join(compiledPagesDir, `${slug}.html`), htmlBody, 'utf8');
}
